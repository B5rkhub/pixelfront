...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════
     6. SAVAŞ BÖLGESİ MODAL — Toolbardan "Savaş Bölgesi" butonuna
        basınca aktif savaşları listeleyen modal aç
  ══════════════════════════════════════════════════════════════ */

  window.wzOpenPickModal = function () {
    const old = document.getElementById('wz-pick-modal');
    if (old) old.remove();

    const wars = (typeof _wars !== 'undefined' && Array.isArray(_wars)) ? _wars : [];

    const modal = document.createElement('div');
    modal.id = 'wz-pick-modal';
    modal.style.cssText = 'position:fixed;inset:0;z-index:900;background:rgba(0,0,0,.75);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;';
    ...
    if (wars.length === 0) {
      ...
      const items = wars.map(w => `
        <div onclick="wzJumpToWarId('${w.id}')"
          style="display:flex;align-items:center;gap:.7rem;padding:.8rem 1rem;border-radius:10px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);cursor:pointer;margin-bottom:.5rem;transition:background .15s;"
          onmouseover="this.style.background='rgba(240,74,74,.15)'"
          onmouseout="this.style.background='rgba(255,255,255,.05)'">
          <span style="font-size:1.3rem;">⚔️</span>
          <div style="flex:1;">
            <div style="font-size:.85rem;font-weight:700;">
              <span style="color:${w.factionA && w.factionA.color ? w.factionA.color : '#f04a4a'}">${_wzEsc((w.factionA && w.factionA.name) ? w.factionA.name : 'A')}</span>
  ...
  window.wzJumpToWarId = function (warId) {
    document.getElementById('wz-pick-modal') && document.getElementById('wz-pick-modal').remove();
    const wars = (typeof _wars !== 'undefined' && Array.isArray(_wars)) ? _wars : [];
    ...
    if (typeof window.showWarZoneBanner === 'function') {
      window.showWarZoneBanner({ war: war, region: war.region });
    }
    if (typeof window.wzJumpToZone === 'function') {
      setTimeout(function() { window.wzJumpToZone(); }, 200);
    }
  };
